...
(function () {
  ...
  function maxCryptoToReport(wantedCoin) {
    ...
    for (const crypto of data) {
      ...
      crypto_listner.addEventListener("click", function () {
        ...
        selectedCrypto.push(wantedCoin);
  ...
  function addCryptoToReport() {
    ...
    for (const crypto of cache) {
      try{
        document.getElementById("check-" + crypto).checked = true;
        ...
        if (priceContainer.className === "mt-1 d-none") {
          ...
          if (check.isExpired === true || check.isExpired === "None") {
            const response = await fetch(
              "https://api.coingecko.com/api/v3/coins/" + btn.id
            );
            const extraData = await response.json();
            savetoLocalStorage(extraData, btn.id);
            priceContainer.className = "d-block mt-1 ";
            const dollars =
              "<p>" + extraData.market_data.current_price.usd + "$" + "</p>";
            const shekels =
              "<p>" + extraData.market_data.current_price.ils + "₪" + "</p>";
            const pounds =
              "<p>" + extraData.market_data.current_price.gbp + "£" + "</p>";
